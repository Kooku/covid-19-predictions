...
object TweetCountCanadaByRetweets {
  ...
  def main(argv: Array[String]) {
    ...
    val conf = new SparkConf().setAppName("Tweet Count Canada By Retweets")
    ...
    val counts = textFile
      .map(line => {
        ...
        ((createdAt, countryCode), if (retweetCount == 0) 1 else retweetCount)
      })
